...
class Song(object):
    ...
    def __init__(self, song_data):
        # Check checksums
        assert song_data.mem_init_flag_1 in (b'rb', 'rb')
        assert song_data.mem_init_flag_2 in (b'rb', 'rb')
        assert song_data.mem_init_flag_3 in (b'rb', 'rb')
...
def _make_property(field_name, doc_string):
    def field_getter(this):
        return getattr(this.song_data, field_name)
    def field_setter(this, value):
        setattr(this.song_data, field_name, value)
    return property(fset=field_setter, fget=field_getter, doc=doc_string)
...
for field, doc in [("tempo", "the song's tempo"),
    ...
    setattr(Song, field, _make_property(field, doc))
